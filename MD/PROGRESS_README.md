...
**Current front (2026-07-23):** **PP2-1a (honest symbolic-split retry) is DONE — ROBUST FAILURE** (`MD/findings/pre_phase2/PP2_1a_symbolic_retry.md`): 0/8 symbolic candidates cleared the benign-per-channel precondition, 0 fusion splits; the leak is triggered by the **object phrase itself** so whichever channel carries it leaks alone, and the only inert audio fragments are too short to pass the intelligibility gate. The ⚖ strategic decision is now **resolved to the sub-symbolic (PGD) route** (hand-crafted symbolic carriers are closed). **Next front = the Rung 2.0 PGD feasibility gate** (`NEXT_STEPS.md`).
...
- **Benign-per-channel split (the crux) — SYMBOLIC ROUTE CLOSED:** the honest symbolic-split retry (PP2-1a, 2026-07-23) **robustly failed** on the hardened 7B instrument — **0/8** candidates cleared the benign-per-channel precondition, **0** genuine fusion splits (`MD/findings/pre_phase2/PP2_1a_symbolic_retry.md`). Root cause: the object phrase itself triggers the leak, so whichever channel carries it leaks alone, and the only inert audio fragments are too short to pass the intelligibility gate. (The earlier `12_symbolic_split/` first run, 2026-07-21, was INCONCLUSIVE on a confounded instrument; PP2-1a is the clean, robust negative.) **A benign-per-channel split cannot be hand-crafted.** The goal is **not abandoned** — it is now gated on **sub-symbolic PGD (Phase 3, Rung 2.0 feasibility gate first)**, which supplies the covert carriers. If PGD is also infeasible, the benign-split deliverable is dropped and the base paper carries the project.
